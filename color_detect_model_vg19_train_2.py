...
from datasets_loader import ImbalancedDatasetSampler
import random
...
def main():
    ...
    training_dir = "/home/mayank_sati/Documents/datsets/Rosbag_files/xshui_all/all_img_sorted"
...
########################################33
    ...
    # train_dataloader = DataLoader(train_dataset, sampler=ImbalancedDatasetSampler(train_dataset), shuffle=True, num_workers=4, batch_size=args.batch_size)
    # # train_dataloader = DataLoader( sampler=ImbalancedDatasetSampler(train_dataset), shuffle=True, num_workers=4, batch_size=args.batch_size)

    train_dataset = torchvision.datasets.ImageFolder(args.train_dir, torchvision.transforms.Compose(
        [torchvision.transforms.RandomResizedCrop(30), torchvision.transforms.RandomHorizontalFlip(),
         torchvision.transforms.ToTensor(), normalize, ]))

    # train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, sampler=None,
    #                                            **kwargs)
    train_dataloader = DataLoader(train_dataset, sampler=ImbalancedDatasetSampler(train_dataset), num_workers=4, batch_size=args.batch_size)
    ...
    if (args.save_model):
        torch.save(model.state_dict(), "color_model_19_on_All_tl_gwm.pt")
