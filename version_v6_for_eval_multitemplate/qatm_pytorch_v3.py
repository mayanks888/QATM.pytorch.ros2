...
class ImageDataset(torch.utils.data.Dataset):
    def __init__(self, template_dir_path, image_name, thresh_csv=None, transform=None):
        ...
        if not self.transform:
            ...
            self.transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean=[0.485], std=[0.229])])
        self.template_path = list(template_dir_path.iterdir())
        self.image_name = image_name
        
        self.image_raw = cv2.imread(self.image_name)
        imgray = cv2.cvtColor(self.image_raw, cv2.COLOR_BGR2GRAY)
        self.image_raw=np.expand_dims(imgray, axis=2)
        # self.image_raw = cv2.merge((imgray, imgray, imgray))
        # self.image_raw = cv2.cvtColor(self.image_raw, cv2.COLOR_BGR2GRAY)
        # self.image_raw=self.image_raw.unsqueeze(0)
        ########3
        # convering into greyscale
...
class ImageDataset_2(torch.utils.data.Dataset):
    def __init__(self, template_dir_path, image_name, thresh_csv=None, transform=None):
        self.transform = transform
        if not self.transform:
        ...
        # self.image_raw=cv2.resize(self.image_raw,(int(1280),int(620)))
...
class Featex():
    def __init__(self, model, use_cuda):
        ...
        ############################33
        # self.model[0] = nn.Conv2d(1, 64, kernel_size=3, stride=1, padding=1)
        ###################################
...
def nms_multi(scores, w_array, h_array, thresh_list):
    ...
    while order.size > 0:
        ...
        inter = w * h
        ovr = inter / (areas[i] + areas[order[1:]] - inter)
        #remember the bigger template you are chosing , then box can tends to overlap over each other, so
        #then you can consider the iou ~.7->9 also but if temp size is small then you can take less value of iou~ .2,.4
    ...
    boxes = np.array([[x1[keep], y1[keep]], [x2[keep], y2[keep]]]).transpose(2, 0, 1)
    scores = scores[keep]
    return boxes, np.array(keep_index),scores
...
def plot_result_multi_univ(image_raw, boxes, indices, show=False, save_name=None, color_list=None):
    d_img = image_raw.copy()
...
if __name__ == '__main__':
    template_dir = '../data/cust_template/'
    image_path = '../data/sample/3.jpg'
    ...
    d_img = plot_result_multi(model,dataset.image_raw, boxes, indices, show=True, save_name='result_sample.png')
